...
describe 'Ita Taxonomy API V2', type: :request do
  ...
  let(:expected_results) { JSON.parse(Pathname.new("#{File.dirname(__FILE__)}/ita_taxonomies/results.json").read, symbolize_names: true) }
  ...
  describe 'GET /ita_taxonomies/search.json' do
    context 'when search parameters are empty' do
      ...
      it 'returns ita taxonomies terms' do
        ...
        # File.open('ita_taxonomies.json', 'w') do |f|
        #   f.puts json_response[:results].to_json
        # end
    ...
    context 'when q is specified' do
      ...
      it 'returns ita taxonomies entries' do
        ...
        expected_result = expected_results.find { |r| r[:label] == 'Peru' }
    ...
    context 'when labels is specified' do
      let(:params) { { labels: 'United States, Africa' } }
      before { get search_path, params, @v2_headers }
      subject { response }
      ...
      it 'returns ita taxonomies entries' do
        ...
        expect(results).to include(expected_results.find { |r| r[:label] == 'United States' })
        expect(results).to include(expected_results.find { |r| r[:label] == 'Africa' })
      end
      it_behaves_like "an empty result when a query doesn't match any documents"
    end
  end
